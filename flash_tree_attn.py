...
def _flash_decoding_fwd_kernel(
    ...
    maskone, #[batch_size, q_len, mask_kv_len]
    masktwo, #[batch_size, q_len, mask_kv_len]
    kv_seq_len,  # [batch_size]
    q_len: tl.constexpr,
    mask_kv_len: tl.constexpr,
    batch_size,
    sm_scale,
    stride_qt,
    stride_qh,
    ...
    cur_s_offset = cur_token_idx * stride_s_b
    S_block_ptr_1 = tl.make_block_ptr(
        base=maskone + cur_s_offset,
        shape=(q_len, mask_kv_len),
        strides=(stride_s_qlen, stride_s_kvlen),
        offsets=(0, 0),
        block_shape=(q_len, mask_kv_len),
        order=(0, 1),
    )

    cur_stwo_offset = cur_token_idx * stride_s_two_b
    S_block_ptr_2 = tl.make_block_ptr(
        base=masktwo + cur_stwo_offset,
        shape=(q_len, mask_kv_len),
        strides=(stride_s_two_qlen, stride_s_two_kvlen),
        offsets=(0, 0),
        block_shape=(q_len, mask_kv_len),
        order=(0, 1),
    )  
    ...
    # apply tree mask
    total_blocks = (cur_kv_seq_len + BLOCK_KV - 1) // BLOCK_KV

    if block_start_kv == total_blocks - 2:
        # last two
        mask = tl.load(S_block_ptr_1)
        S_ij = tl.where(mask, S_ij, float("-inf"))

    elif block_start_kv == total_blocks - 1:
        # last one
        mask = tl.load(S_block_ptr_2)
        S_ij = tl.where(mask, S_ij, float("-inf"))
...
def flash_decoding_attention(
    ...
    Args:
        ...
        tree_mask: (torch.tensor): [bsz, num_heads, q_len, kv_len], mask will apply to the last kv_len of cache
    ...
    append_len = current_k.size(2)
    insert_indices = kv_seq_len.unsqueeze(-1) + torch.arange(append_len, device=kv_seq_len.device).unsqueeze(0)
    insert_indices = insert_indices[:, None, :, None].expand(-1, k_cache.size(1), -1, head_dim)

    k_cache.scatter_(2, insert_indices, current_k)
    v_cache.scatter_(2, insert_indices, current_v)

    kv_seq_len += append_len

    tree_mask_1 = tree_mask.new_ones((bsz, q_len, block_size))
    tree_mask_2 = tree_mask.new_ones((bsz, q_len, block_size))

    last_len = kv_seq_len % block_size
    mask_kv_len = tree_mask.size(-1)
    ...
    _flash_decoding_fwd_kernel[grid](
        ...
        tree_mask_1.stride(0),
        tree_mask_1.stride(1),
        tree_mask_1.stride(2),
        tree_mask_2.stride(0),
        tree_mask_2.stride(1),
        tree_mask_2.stride(2),
...
def torch_tree_attention(q, k, v, tree_mask, sm_scale):
    p = torch.matmul(q, k.transpose(2, 3)) * sm_scale
    new_tree_mask = tree_mask.int()
    float_tree_mask = tree_mask.float()
    float_tree_mask[new_tree_mask.eq(0)] = float('-inf')
    float_tree_mask[new_tree_mask.eq(1)] = 0
    mask = p.new_zeros(p.size())
    mask[..., -tree_mask.size(0):, -tree_mask.size(1):] += float_tree_mask
    p += mask
    # p[:, :, :, -32] = float('-inf')
    lse = p.logsumexp(dim=-1, keepdim=True)
    p = torch.softmax(p.float(), dim=-1).half()
...
# If using CUDA, also set the seed for CUDA
if torch.cuda.is_available():
    torch.cuda.manual_seed(42)
    torch.cuda.manual_seed_all(42)  # If using multi-GPU

# Optional: Ensure deterministic behavior for cuDNN
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False

@pytest.mark.parametrize("Z, H, N_CTX, HEAD_DIM, q_len", [(1, 32, 4096, 128, 32)])
def test_tree_op(Z, H, N_CTX, HEAD_DIM, q_len):
    ...
    assert  torch.allclose(ref_out, out, atol=1e-3, rtol=0)
